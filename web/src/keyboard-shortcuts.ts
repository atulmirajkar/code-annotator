...
export function bindKeyboardShortcuts(
  ...
  const dialog = requiredElement(
    document.querySelector<HTMLDialogElement>(".shortcuts-dialog"),
    "shortcuts dialog",
  );
  const openButton = requiredElement(
    document.querySelector<HTMLButtonElement>(".shortcuts-open"),
    "shortcuts open button",
  );
  const closeButton = requiredElement(
    document.querySelector<HTMLButtonElement>(".shortcuts-dialog-close"),
    "shortcuts close button",
  );
  const enabledCheckbox = requiredElement(
    document.querySelector<HTMLInputElement>(".shortcuts-enabled-toggle"),
    "shortcuts enabled checkbox",
  );
  const status = requiredElement(
    document.querySelector<HTMLElement>(".shortcut-status"),
    "shortcut status region",
  );

  const stored = readPreference(storage, shortcutsEnabledStorageKey);
  const context: KeyboardShortcutsContext = {
    document,
    window,
    storage,
    dialog,
  ...
  enabledCheckbox.addEventListener("change", () => {
    ...
    writePreference(
      ...
      String(context.enabled),
    );
    if (!context.enabled) cancelLeader(context);
  });
  document.addEventListener("keydown", (event) =>
    handleKeydown(context, event),
  );
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) cancelLeader(context);
  });
}

function requiredElement<T extends Element>(value: T | null, label: string): T {
  if (!value) throw new Error(`keyboard shortcuts: missing ${label}`);
  return value;
}

function openShortcutsDialog(context: KeyboardShortcutsContext): void {
...
function handleIdleKey(
  context: KeyboardShortcutsContext,
  event: KeyboardEvent,
): void {
  if (!context.enabled) return;
  if (isSuppressedTarget(context, event.target)) return;
...
function isSuppressedTarget(
  context: KeyboardShortcutsContext,
  target: EventTarget | null,
): boolean {
  if (!(target instanceof HTMLElement)) return false;
  if (context.dialog.contains(target)) return true;
...
function armLeader(context: KeyboardShortcutsContext): void {
  context.leaderTimer = context.window.setTimeout(
    () => cancelLeader(context),
    leaderTimeoutMs,
  );
...
// after).
function cancelLeader(context: KeyboardShortcutsContext): void {
  if (context.leaderTimer !== null) {
  ...
  switch (command) {
    ...
    case "add-comment":
      context.document.dispatchEvent(
        new CustomEvent("code-annotator:add-comment"),
      );
      return;
  }
}

function announcePanelToggle(
  context: KeyboardShortcutsContext,
...
function activatePanelToggle(
  ...
  const hidingFocusedPanel =
    panel !== null && !panel.hidden && panel.contains(document.activeElement);
  button.click();
  if (hidingFocusedPanel) button.focus();
