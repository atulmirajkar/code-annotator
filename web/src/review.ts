...
function selectionInput(form: HTMLFormElement, name: string): HTMLInputElement | null {
  const value = form.elements.namedItem(name);
  return value instanceof HTMLInputElement ? value : null;
}

function writeSelection(form: HTMLFormElement, selection: SelectionPayload | null): void {
  const values = {
...
async function currentDocumentPath(location: Location): Promise<string> {
  ...
  if (!state.selectedPath) throw new Error("Review page has no selected document");
...
export async function initializeReview(environment: ReviewEnvironment = { document, window }): Promise<void> {
  ...
  const panelController = createReviewPanelController({
    ...
    form,
    formStatus,
    addAnnotationButton: panel.querySelector<HTMLButtonElement>(".add-annotation-toggle"),
    closeAnnotationButton: panel.querySelector<HTMLButtonElement>(".annotation-form-close"),
    layout: panel.closest<HTMLElement>(".layout"),
    resizeHandle: panel.querySelector<HTMLElement>(".review-panel-resize"),
    documentPath,
  });
  ...
  function updateReattachForms(): void {
    reviewPanel.querySelectorAll<HTMLFormElement>(".annotation-reattach").forEach((reattachForm) => {
      const selection = currentSelection();
      if (selection) writeSelection(reattachForm, selection);
      ...
      const button = reattachForm.querySelector<HTMLButtonElement>('button[type="submit"]');
      if (button) button.disabled = !ready;
      const help = reattachForm.querySelector<HTMLElement>(".reattach-help");
      if (help) help.textContent = ready
  ...
  function displayedAnnotations(): AnnotationBrowserState[] {
