...
// Reply and reattach/lifecycle/quick-close forms all replace
// #annotation-panel-content, which drops every card's expanded state and
// would otherwise strand focus on <body>. AnnotationOperationKind names which
// form kind was submitted so restoreAfterSwap can reopen the right disclosure
// and find the right status text.
type AnnotationOperationKind =
  | "reply"
  | "reattach"
  | "lifecycle"
  | "quickClose";

interface PendingAnnotationOperation {
  cardID: string;
  kind: AnnotationOperationKind;
}

// Focus-restoration handle returned by bindAnnotationCardDisclosures. The
// review HTMX adapter calls restoreAfterSwap once per completed request,
// after the replacement fragment has been swapped into the DOM.
export interface AnnotationOperationFocus {
  restoreAfterSwap(successful: boolean): void;
}
...
export function bindAnnotationCardDisclosures(
  panel: HTMLElement,
  signal: AbortSignal,
): AnnotationOperationFocus {
  ...
  let pendingOperation: PendingAnnotationOperation | null = null;
  ...
  return {
    restoreAfterSwap(successful) {
      ...
      if (operation) restoreOperationFocus(panel, operation, successful);
    },
  };
}
...
function handleClick(panel: HTMLElement, event: MouseEvent): void {
  const target = event.target instanceof Element ? event.target : null;
  const button = target?.closest<HTMLButtonElement>(toggleSelector);
  if (!button || !panel.contains(button)) return;

  const disclosurePanel = disclosurePanelFor(panel, button);
  if (!disclosurePanel) return;
...
function handleKeydown(panel: HTMLElement, event: KeyboardEvent): void {
  if (event.key !== "Escape") return;
  const target = event.target instanceof Element ? event.target : null;
  const disclosurePanel = target?.closest<HTMLElement>(panelSelector);
  if (
    !disclosurePanel ||
...
function toggleForPanel(
  ...
  for (const button of panel.querySelectorAll<HTMLButtonElement>(
    ...
    if (button.getAttribute("aria-controls") === panelID) return button;
  }
  return null;
}
...
function operationKindForForm(
  form: HTMLFormElement,
): AnnotationOperationKind | null {
  if (form.classList.contains("annotation-reply")) return "reply";
  if (form.classList.contains("annotation-reattach")) return "reattach";
  if (form.classList.contains("annotation-lifecycle")) return "lifecycle";
...
function pendingOperationFromSubmit(
  panel: HTMLElement,
  event: SubmitEvent,
): PendingAnnotationOperation | null {
  const form = event.target instanceof HTMLFormElement ? event.target : null;
  if (!form || !panel.contains(form)) return null;
  const card = form.closest<HTMLElement>(".annotation-card");
...
function restoreOperationFocus(
  panel: HTMLElement,
  operation: PendingAnnotationOperation,
  successful: boolean,
): void {
  ...
  if (!focusPanelFeedback(documentRef)) focusCardSummary(card);
}

function focusCardSummary(card: HTMLElement): void {
  card.querySelector<HTMLElement>(".annotation-summary")?.focus();
}

function focusPanelFeedback(documentRef: Document): boolean {
  const feedback = documentRef.querySelector<HTMLElement>(
    ".annotation-panel-feedback",
  );
  feedback?.focus();
...
function reopenOperationDisclosure(
  panel: HTMLElement,
  card: HTMLElement,
  kind: AnnotationOperationKind,
): void {
  ...
  const button = card.querySelector<HTMLButtonElement>(`.${toggleClass}`);
  if (!button) return;
  const disclosurePanel = disclosurePanelFor(panel, button);
  if (disclosurePanel) setExpanded(button, disclosurePanel, true);
}
