...
(() => {
  ...
  if (!layout?.classList.contains("layout")) return;

  try {
    if (
      sessionStorage.getItem("code-annotator.panel-collapsed.documents") ===
      "true"
    ) {
      layout.classList.add("documents-collapsed");
    }

    const annotations = sessionStorage.getItem(
      "code-annotator.panel-collapsed.annotations",
    );
    if (
      layout.classList.contains("review-layout") &&
      (annotations === null || annotations === "true")
    ) {
      layout.classList.add("review-collapsed");
    }

    const documentScope = sessionStorage.getItem(
      "code-annotator.document-scope",
    );
    if (
      documentScope === "all" ||
