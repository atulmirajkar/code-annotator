...
export type DocumentMode = "file" | "diff";
...
export interface DocumentCatalogState {
  ...
  documentsByPath: ReadonlyMap<string, DocumentCatalogItem>;
}
...
function parseItem(value: unknown, index: number): DocumentCatalogItem {
  ...
  if (!path || !name) throw new Error("document path and name must not be empty");
  if (path.startsWith("/") || path.includes("\\") || path.split("/").some((segment) => segment === "" || segment === "." || segment === "..")) {
    throw new Error("document path is invalid");
  }
  if (directory !== (path.includes("/") ? path.slice(0, path.lastIndexOf("/")) : "")) throw new Error("document directory is inconsistent");
  if (!url.startsWith("/view/")) throw new Error("document URL is invalid");
  return {
    path,
    name,
    directory,
...
export function parseDocumentCatalogState(value: unknown): DocumentCatalogState {
  const root = requireRecord(value, "document state");
  if (root.schemaVersion !== 1) throw new Error("document state schemaVersion is unsupported");
  if (!Array.isArray(root.documents)) throw new Error("documents must be an array");
  ...
  if (changedAvailable && changedError) throw new Error("changed state cannot be available and failed");

  const documents = root.documents.map(parseItem);
  const documentsByPath = new Map<string, DocumentCatalogItem>();
  documents.forEach((item) => {
    if (documentsByPath.has(item.path)) throw new Error("document paths must be unique");
  ...
  if (!changedAvailable && documents.some((item) => item.changed)) throw new Error("changed documents require available changed state");
...
export async function fetchDocumentCatalogState(documentPath = "", mode: DocumentMode = "file"): Promise<DocumentCatalogState> {
  const query = new URLSearchParams();
  if (documentPath) query.set("document", documentPath);
  ...
  if (!response.ok) throw new Error(`document state request failed: ${response.status}`);
