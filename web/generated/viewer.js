...
function bindDocumentNavigationLifecycle(environment) {
    environment.document.addEventListener("htmx:afterSettle", (event) => {
        if (!documentSwapTarget(event))
            return;
        bindComparisonControl(environment.document);
        bindDiffDivider(environment.document, environment.storage);
        bindDiffOverview(environment);
        environment.document.dispatchEvent(new CustomEvent("code-annotator:viewer-navigated"));
    });
}
function documentSwapTarget(event) {
...
function configureHTMX(api, document) {
    if (!api)
        return;
    ...
    document.body.addEventListener("htmx:configRequest", (event) => {
        if (Reflect.get(globalThis, "mermaid") === undefined)
            return;
        if (!(event instanceof CustomEvent) || typeof event.detail !== "object" || event.detail === null)
            return;
