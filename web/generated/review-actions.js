...
export function createAnnotationActions({ documentPath, reviewToken, getCurrentRevision, currentSelection, forceClearSelectionPreview, loadAnnotations, setFormStatus, reviewerAuthor, list, }) {
    ...
    function updateReattachControls() {
        list.querySelectorAll(".annotation-reattach").forEach((reattach) => {
            const button = requiredElement(reattach.querySelector('button[type="submit"]'), "reattach button");
            const help = requiredElement(reattach.querySelector(".reattach-help"), "reattach help");
            const selectedRange = currentSelection();
            button.disabled = !selectedRange;
            help.textContent = selectedRange
                ? "Use the current selection as the replacement anchor."
    ...
    async function submitReattach(event, annotationID) {
        ...
        const reattach = requiredElement(event.currentTarget, "reattach form");
        const button = requiredElement(reattach.querySelector('button[type="submit"]'), "reattach button");
        const status = requiredElement(reattach.querySelector(".reattach-status"), "reattach status");
        const selectedRange = currentSelection();
        if (!selectedRange) {
        ...
        try {
            const payload = { document: documentPath, selection };
    ...
    async function submitReply(event, annotationID) {
        event.preventDefault();
        const reply = requiredElement(event.currentTarget, "reply form");
        const button = requiredElement(reply.querySelector('button[type="submit"]'), "reply button");
        const status = requiredElement(reply.querySelector(".reply-status"), "reply status");
    ...
    async function submitLifecycle(event, annotationID) {
        event.preventDefault();
        const lifecycle = requiredElement(event.currentTarget, "lifecycle form");
        const button = requiredElement(lifecycle.querySelector('button[type="submit"]'), "lifecycle button");
        const status = requiredElement(lifecycle.querySelector(".lifecycle-status"), "lifecycle status");
        const fields = new FormData(lifecycle);
        const selected = requiredElement(lifecycle.elements.namedItem("status")?.selectedOptions[0] || null, "lifecycle action");
        const activityKind = selected.dataset.activity;
        const payload = {
            document: documentPath,
        ...
        if (activityKind === "message")
            payload.message = String(fields.get("activity") || "");
        if (activityKind === "summary") {
            payload.summary = String(fields.get("activity") || "");
            const commit = fields.get("commit");
            if (commit)
                payload.commit = String(commit);
        }
