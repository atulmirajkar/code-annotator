...
export function effectiveTheme(document, window) {
...
export function bindThemeToggle(document, window, storage) {
    const toggle = document.querySelector(".theme-toggle");
    if (!toggle)
        return;
    const stored = readPreference(storage, themePreferenceKey);
    if (isViewerTheme(stored))
    ...
    toggle.addEventListener("click", () => {
        const theme = effectiveTheme(document, window) === "dark"
            ? "light"
            : "dark";
