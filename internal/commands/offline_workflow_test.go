...
func TestOfflineAnnotationWorkflow(t *testing.T) {
	...
	steps := []struct {
		...
		{name: "acknowledge work", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "acknowledged", "--role", "agent", "--author", "codex")...)},
		{name: "ask a question", args: append([]string{"reply"}, append(common, "--id", "ann_handoff", "--author", "codex", "--message", "Should the example retain compatibility wording?")...)},
		{name: "report first attempt", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "applied", "--role", "agent", "--author", "codex", "--summary", "Updated the behavior and example.", "--commit", "abc1234")...)},
		{name: "reviewer requests changes", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "needs_changes", "--role", "reviewer", "--author", "atul", "--message", "Retain the compatibility wording.")...)},
		{
		...
		{name: "acknowledge retry", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "acknowledged", "--role", "agent", "--author", "codex")...)},
		{name: "report second attempt", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "applied", "--role", "agent", "--author", "codex", "--summary", "Restored compatibility wording.", "--commit", "def5678")...)},
		{name: "agent cannot close", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "closed", "--role", "agent", "--author", "codex")...), wantErr: "cannot transition"},
		{name: "reviewer closes", args: append([]string{"resolve"}, append(common, "--id", "ann_handoff", "--status", "closed", "--role", "reviewer", "--author", "atul")...)},
