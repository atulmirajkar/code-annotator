...
`npm run check:web`; viewer pages intentionally do not load it before the
activation gate. Its npm provenance, license, and SHA-256 verification command
...
The complete server-rendered annotation mutation surface is compiled and
tested but remains inactive in the page until the HTMX activation milestone.
